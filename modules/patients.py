...
class PatientManager:
    ...
    def get_patient_detailed_statement(self, patient_id):
        ...
        cigarettes_cost = 0
        if patient[6]:
            cigarette_price_query = "SELECT setting_value FROM settings WHERE setting_key = 'cigarette_pack_price'"
            price_result = self.db.fetchone(cigarette_price_query)
            cigarette_price = float(price_result[0]) if price_result else 40
            cigarettes_cost = days * patient[7] * cigarette_price
